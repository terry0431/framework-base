...
var JCTools = {
    ...
    checkNum:function (intLength, floatLength) {
        ...
        var isInt = new RegExp("^(([1-9]\\d{0," + (intLength - 1) + "}))$");
        var isFloat = new RegExp("^((0\\.\\d{0," + floatLength + "})|([1-9]\\d{0," + (intLength - 1) + "}\\.\\d{0," + floatLength + "}))$");
...
// Process the coversion from currency digits to characters:
    ...
    intDeal:function (e) {
        ...
        if (eType == "blur" || eType == "mouseout") {
            ...
            if((this.value=="0"||this.value=="")&&defaultValue!=undefined){this.value=defaultValue}
        }
    },
    intInit:function(inputs){
        inputs = inputs == undefined ? $("input[itype=int]").unbind(".intDeal") : inputs;
        inputs.bind("keydown.intDeal keyup.intDeal blur.intDeal", JCTools.intDeal);
        return this
    },
    /**
    ...
    floatDeal:function (e) {
        ...
        if (eType == "keydown") {
            ...
            if ((key <= 57 && key >= 48) || (key >= 96 && key <= 105)) {
                ...
                return (max == undefined || this.value != max) && (precision == undefined || this.value.split(".").length<2||(this.value.split(".")[1]).length != parseInt(precision,10)||JCTools.getPositionForInput(this)<this.value.indexOf(".")+1)
            }
            if (key == 110 || key == 190) {
    ...
    floatInit:function(inputs){
        ...
        inputs.bind("keydown.floatDeal keyup.floatDeal blur.floatDeal", JCTools.floatDeal);
        return this;
    },
...
//    } ,
     ...
     * @example  JCTools.enterToTab() , JCTools.enterToTab($("input:visible,select,a.carpo-search-button"))
     */
    enterToTab:function(inputs) {
        if (inputs == undefined) {
            inputs = $("input:visible,select");
        }
        ...
        inputs.unbind(".entertotab").bind("keydown.entertotab", function (e) {
            if (e.which == 13) { //判断所按是否回车键
                ...
                var topIframe = top.document.getElementsByTagName("iframe");
//                if (topIframe.length>0&&topIframe[0].src.indexOf("My97DatePicker.htm") != -1) {
...
String.prototype.dateFormat = function() {
    ...
    var dateArr;
    var timeArr;
    if (trimValue.length === 10) { //date 2010-15-21
...
var JCCheckFunc = {
    ...
    checkEmpty:function () {
     ...
     /**
     * @description 两个input value 的大小关系
     * @param {String} objId_first 要比较的结点id 1
     * 
     * @param {String} objId_second 要比较的结点id 2
     ...
     */
    checkIsMoreThan:function(){
        arguments = arguments[0];
        var objId_first = arguments[0];
        var objId_second = arguments[1];
        var objText = arguments[2];
        var obj_first = $("#" + objId_first);
        var obj_second = $("#" + objId_second);
        var returnObj = $("#" + objId_first).val().isNotEmpty();
        returnObj.obj = obj_first;
        var num_first = Number(obj_first.val());
        var num_second = Number(obj_second.val());
        if( num_first < num_second){
        	returnObj.errMsg = objText == undefined||objText == "" ? "输入非法！" : objText ;
        	returnObj.bool = false;
        }
        return returnObj;
    },
    ...
    compareDate:function () {
        ...
        if (startDateObj.val().dateFormat() > endDateObj.val().dateFormat()) {
            returnObj.bool = false;
            returnObj.errMsg = startDateText + "不能大于" + endDateText;
        } else {
            returnObj.bool = true;
        }
        return returnObj;
        ...
        var ccjc = (function () {
            ...
            ccjc.fn = ccjc.prototype = {
                ...
                methods:{
                    ...
                    checkIsMoreThan:JCCheckFunc.checkIsMoreThan,
                ...
                init:function () {
                    if (!window.XMLHttpRequest) {
                        ...
                        JCTools.firstFocus();
                        this.enterToTabObj = JCTools.enterToTab();
                        JCTools.forbidBack();
                    }
                    return this;
                },
...
//                    flag&&this.inputTrim();
    ...
    JC.tableHandle={
        buttonHandle: function ($html, opt) {
            for (var i in opt.buttonList) {
            //for (var i = 0;i < opt.buttonList.length;i ++) {
                var buttonObj = opt.buttonList[i];
                //解决 ie cms系统 新闻列表 会出现 undefined 按钮
                if(buttonObj.name == undefined){
                	buttonObj.name = '';
                }
                var li = $('<li class="' + (buttonObj.className == undefined ? '' : buttonObj.className) + '" ' +
                        'style="' + (buttonObj.style == undefined ? '' : buttonObj.style) + '">' + buttonObj.name + '</li>');
                if (buttonObj.event != undefined) {
                    li.bind(buttonObj.event, {customArg:buttonObj.customArg}, buttonObj.eventFunc);
                }
                $html.find("ul.buttonList").append(li);
            }
        },
        titleHandle:function ($html, opt) {
            var headTr = $html.find("tr.headtr");
            for (var i = 0, j = opt.colModel.length; i < j; i++) {
                var colObj = opt.colModel[i];
                var tdStr = '<td style="width:' + colObj.width + ';text-align:' + (colObj.align == undefined ? 'center' : colObj.align) + ';'
                        +(colObj.style == undefined ? '' : colObj.style)+'" ' +
                        ' class="' + (colObj.className == undefined ? '' : colObj.className) + '">' +
                        '<div class="th_wrapp">' +
                            '<span>' + colObj.value + '</span>';
                if(colObj.order){
                    tdStr+= '<span style="width:13px;height:10px;display: inline-block;margin-left: 10px;cursor:pointer;" class="unsorted" name="sort_span_'+colObj.name+'"></span>'
                }
                tdStr+='</div></td>';
                var td = $(tdStr);
                if (colObj.event != undefined) {
                    td.css({cursor:"pointer"}).bind(colObj.event,{arg:colObj,customArg:colObj.customArg},colObj.eventFunc);
                }
                var sortSpan = td.find("span[name^=sort_span]");
                sortSpan.add(sortSpan.siblings()).css("cursor","pointer").click(function(){
                    var $this = $(this).parent("div.th_wrapp").find("span[name^=sort_span]");
                    opt.sortField=$this.attr("name").replace("sort_span_","");
            ...
            headTr.find("span[name=sort_span_"+opt.sortField+"]").removeClass().addClass(opt.sortOrder);
        },
        bodyHandle:function ($html,opt){
            var tbody=$("<tbody></tbody>");
            for (var i = 0, ii = opt.data.length; i < ii; i++) {
                var temp = i % 2 == 0 ? 'odd' : 'even';
                var rowObj = opt.data[i];
                var tr = $('<tr class="' + temp + '" oriClass="'+temp+'"></tr>')
                    .hover(
                        function(){
                            $(this).removeClass("odd even").addClass("mover");
                        },
                        function(){
                            var $this = $(this);
                            $this.removeClass("mover");
                            $this.hasClass("click") || $this.addClass($this.attr("oriClass"));
                        }
                    )
                    .click(function () {
                        var $this = $(this);
                        if (!$this.hasClass("click")) {
                            $this.removeClass("odd even mover").addClass("click").find("input:checkbox:first:enabled").attr("checked", "checked");
                        } else {
                            $this.removeClass("click").addClass($this.attr("oriClass")).find("input:checkbox:first:enabled").removeAttr("checked");
                        }
                    });
                if(rowObj.event!=undefined){
                	
                    tr.bind(rowObj.event,{arg:rowObj,customArg:obj.customArg},rowObj.eventFunc);
                }
                for(var j=0,jj=opt.colModel.length;j<jj;j++){
                    var col = opt.colModel[j];
                    var colName = col.name;
                    var cell = rowObj.cell;
                    var cellValue = cell[colName].length == undefined ? cell[colName].value : cell[colName].value.substring(0,cell[colName].length)+"…";
                    cellValue = cell[colName].format == undefined ? cellValue : cell[colName].format(cellValue, cell);
                    var isshow = cell[colName].isshow;
                    if(isshow == "0"){
                    	cellValue = "";
                    }
                    var cellWidth="";
                    if(i==0){
                        cellWidth = 'width:' + col.width+';';
                    }
                    
                    var td=$('<td style="'+cellWidth+'text-align:' + (cell[colName].align == undefined ? 'left' : cell[colName].align) + ';' +
                            (cell[colName].style == undefined ? '' : cell[colName].style)+'" ' +
                            ' class="' + (cell[colName].className == undefined ? '' : cell[colName].className) + '" '+
                            (cell[colName].title == undefined ? '' : 'title="' + cellValue + '"') + '>' + cellValue + '</td>');
                    if (cell[colName].event != undefined && isshow != "0") {
                        td.css({cursor:"pointer"}).bind(cell[colName].event, {arg:rowObj,customArg:cell[colName].customArg}, cell[colName].eventFunc);
                    }
                    tr.append(td);
                }
                tbody.append(tr);
            }
            $html.find("table.dataTable").append(tbody);
        },
        footerHandle:function ($html,opt){
            var pageTotal = Math.ceil(opt.rowTotal/opt.pageSize);
            var footLi = '<li>高度<select class="listHeight pageup Sideline mlr5" style="width:50px;padding:0">';
            var listHeightOption = [];
            for(var i=0,j=opt.listHeightArray.length;i<j;i++){
                listHeightOption[i]="<option value='"+opt.listHeightArray[i]+"'>"+opt.listHeightArray[i]+"</option>";
            }
            var pageSizeOption = [];
            for(var i=0,j=opt.pageSizeArray.length;i<j;i++){
                pageSizeOption[i]="<option value='"+opt.pageSizeArray[i]+"'>"+opt.pageSizeArray[i]+"</option>";
            }
            footLi =footLi +listHeightOption.join("")+'</select></li>' +
                '<li>每页<select class="pageSize pageup Sideline mlr5" style="width:50px;padding:0">'
                + pageSizeOption.join("")+'</select>条</li>' +
                '<li>共<cite class="yellow mlr5">'+opt.rowTotal+'</cite>条</li>' +
                '<li>共<cite class="yellow mlr5">' + pageTotal + '</cite>页</li>' +
                '<li style="cursor: pointer" pageNum="1">首页</li>' +
                '<li style="cursor: pointer" pageNum="'+(opt.pageNum>1?opt.pageNum-1:1)+'">上一页</li>' +
                '<li style="cursor: pointer" pageNum="'+(opt.pageNum<pageTotal?parseInt(opt.pageNum,10)+1:pageTotal)+'">下一页</li>' +
                '<li style="cursor: pointer" pageNum="' + pageTotal + '">尾页</li>' +
                '<li>到第<select class="pageNum pageup Sideline mlr5" style="width:50px;padding:0">';
            var optionArr=[];
            for(var i=1;i<pageTotal+1;i++){
                optionArr.push("<option value='"+i+"' "+(i==opt.pageNum?"selected='selected'":"")+">"+i+"</option>");
            }
            footLi = $(footLi + optionArr.join("") + '</select>页</li><li style="cursor: pointer" class="go"><cite class="yellow">GO</cite></li>');
            footLi.filter("li[pageNum]").bind("click",function () {
                goto1(opt,$(this).attr("pageNum"));
            }).end().filter("li.go").bind("click", function () {
                goto1(opt,footLi.find("select.pageNum").val());
            }).end().find("select.listHeight").val(opt.listHeight).bind("change",function(){
                top.globalParam.listHeight = $(this).val();
                JC.table(opt);
            }).end().find("select.pageSize").val(opt.pageSize).bind("change",function(){
                top.globalParam.pageSize = $(this).val();
                JC.table(opt);
            });
            $html.find("ul.page").append(footLi);
            function goto1(opt,pageNum){
                if (opt.pageNum != pageNum){
                    opt.pageNum = pageNum;
                    JC.table(opt);
                }
            }
    ...
    JC.table = function (opt) {
    	if(opt.dataHandle==undefined){
    		opt.dataHandle=dataHandle;
    	}
        opt = $.extend({
            arg:{},
            pageNum:1,
            pageSize:10,
            pageSizeArray:[10,20,30,40,50,100,200,300,500],
            rowTotal:-1,
            listHeight:300,
            listHeightArray:[200,300,400,500,600],
            sortField:"",
            sortOrder:""
           // ,dataHandle:dataHandle
        }, opt || {});
        opt.arg.sortField = opt.sortField;
        opt.arg.sortOrder = opt.sortOrder;
        var that = this;
        if (top.globalParam == undefined) {
            top.globalParam = {}
        }
        if(top.globalParam.pageSize==undefined){
            top.globalParam.pageSize = opt.pageSize;
        }else{
            opt.pageSize=top.globalParam.pageSize;
        }
        if(top.globalParam.listHeight==undefined){
            top.globalParam.listHeight = opt.listHeight;
        }else{
            opt.listHeight=top.globalParam.listHeight;
        }
        tableCover(undefined,$(opt.container));
        //opt.method(opt.arg,opt.pageNum,opt.pageSize,opt.rowTotal,opt.callbackFn,function (data) {
        //})
        $.ajax({
            url: opt.method,
            type: 'POST',
            async: false,
            data: {arg:JSON.stringify(opt.arg),pageNum:opt.pageNum,pageSize:opt.pageSize,rowTotal:opt.rowTotal},
            timeout: fw_config.ajax_time,
            success: function (data) {
                opt.rowTotal = data.rowTotal;
                var list = data.dataList;
                opt.data = opt.dataHandle(list,opt);
                var $html = $('<h3 class="box_c_tit">'+opt.caption+'</h3>' +
                        '<div class="box_tab">' +
                            '<div class="b_tb_btn">' +
                                '<span class="b_tb_tit tlrline left">'+opt.head+'</span>' +
                                '<ul class="b_tb_tit tlrline right buttonList"></ul>' +
                            '</div>' +
                            '<div class="b_tb_c">' +
                                '<div class="titleDiv"><table class="Lst_c_t titleTable"><thead><tr class="headtr"></tr></thead></table></div>' +
                                '<div class="lis_con"><table class="Lst_c_t dataTable"></table></div>' +
                                '<table class="Lst_c_t"><tr class="row"><td colspan="10">' +
                                    '<ul class="page"></ul>' +
                                '</td></tr></table>' +
                            '</div>' +
                        '</div>');
                that.tableHandle.buttonHandle($html,opt);
                that.tableHandle.titleHandle($html,opt);
                that.tableHandle.bodyHandle($html,opt);
                that.tableHandle.footerHandle($html,opt);
                var listDiv = $(opt.container).html($html).find("div.lis_con");
                var $titleDiv = $(opt.container).find("div.titleDiv");
                if (document.all&&JCTools.getIEVersion()!="9") {
                    $titleDiv.width(listDiv.width());
                    $(window).bind("resize", function () {
                        $titleDiv.width(listDiv.width())
                    })
                }

                if(listDiv.height()>opt.listHeight){
                    listDiv.height(opt.listHeight).css({overflowY:"scroll"});
                    $titleDiv.addClass("lis_right");
                }
                JC.tableOpt = opt;
                hideCover();
                if(opt.callbackFn!=undefined){
                    opt.callbackFn();
                }
            },
            error: function (data) {
    ...
    function tableCover(content,$div) {
        content = content==undefined?"内容加载中，请稍等...":content;
        if ($("#tableCover").size() == 0) {
            var html = '<div id="tableCover" class="coverDiv"></div>' +
                    '<span id="tableCoverSpan" class="coverSpan"></span>';
            $("body").append(html);
        } else if ($("#tableCover")[0].style.display != "none") {
            $("#tableCover,#tableCoverSpan").hide();
            return;
        }
        $("#tableCoverSpan").html(content);
