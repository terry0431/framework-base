...
J.dialog = function( opts )
{
	...
	SetDialog = function( dg )
	{
		...
		if( opt.page )
		{
			...
			J(S.dgFrm).bind('load',function(){
                ...
                S.dgWin.JC&&S.dgWin.JC.init();
                var height = S.dgWin.document.documentElement.clientHeight;
                S.dgWin.$("#page").css("height", height - 3.1);
			});
		}
	...
	S.cancel = function()
	{
        ...
        /**若不使用parent属性时，当弹出多个窗口并关闭时，总是保留遮罩直到最后一个窗口关闭*/
        if (opt.parent == null && window.top.$("div.lhgdialog_default").size() > 1) {
            window.top.$("#lhgdgCover").show().css({zIndex:1976})
        }
        /**修改结束*/
	};
	...
	S.cleanDialog = function()
	{
        ...
        if(J("div.lhgdialog_default").size()!=0){
            J("#lhgdgCover").show().css({zIndex:1976})
        }
	};
